				...
				<div class="order-form-card-row">
					...
					<label class="flex-100">Lieferzeitraum *
						<select name='timeslot' id="timeslot" onchange='enableSubmit(this)'>
							...
							<option <?php if(mysqli_fetch_row(mysqli_query($db, "SELECT (COUNT(slot) < 26) as free FROM orders WHERE deleted = 0 AND DATE(date) = '" . date('Y-m-d') . "' AND slot = 0"))[0] == 0) echo 'disabled';?> value="0">18:00 - 19:30</option>
							<option <?php if(mysqli_fetch_row(mysqli_query($db, "SELECT (COUNT(slot) < 26) as free FROM orders WHERE deleted = 0 AND DATE(date) = '" . date('Y-m-d') . "' AND slot = 1"))[0] == 0) echo 'disabled';?> value="1">19:30 - 21:00</option>
						</select>
					</label>
				</div>
				<label>Anmerkungen
